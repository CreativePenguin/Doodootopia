...
public class DrivetrainDriveCommand extends DrivetrainCommand {
    ...
    protected void setTurn() {
        ...
        newTurn *= Drivetrain.Command.TurnSpeed.MAX;

        if(SmarterDashboard.getBoolean("SMOOTH_TURN", Drivetrain.Command.SmoothTurn.ENABLED)) {
    ...
    private void useSmoothTurn(double newTurn) {
        if(mQuickTurn) {
            double weight = (Math.abs(mTurn) < Math.abs(newTurn))
                ? SmarterDashboard.getNumber("SMOOTH_ACCEL", Drivetrain.Command.SmoothTurn.ACCEL)
                : SmarterDashboard.getNumber("SMOOTH_DECEL", Drivetrain.Command.SmoothTurn.DECEL);
    ...
    protected void setQuickTurn() {
        mQuickTurn = Math.abs(mSpeed) < SmarterDashboard.getNumber("QUICKTURN_THRESHOLD", 
                                      Drivetrain.Command.QuickTurn.THRESHOLD);
        
        if (mQuickTurn) {
            mTurn *= SmarterDashboard.getNumber("QUICKTURN_SPEED", 
                    Drivetrain.Command.QuickTurn.SPEED);
        }
    }
}
